...
Always send `destination.type=web|app`: web uses `web_url`; app uses at least one of `app_id`, `store_url`, or `deep_link` and no `web_url`. To select this upload batch, call `creatives_list` with timezone-aware `created_from` (inclusive) and `created_to` (exclusive). At terminal status, require `result.create_reconciliation.reconciled=true` and report each `ad_name` plus available `selection_key`/`selection_keys` and `ad_id`. Treat `recovered_by_url_fallback` as compensated, not a reason to recreate.
```
