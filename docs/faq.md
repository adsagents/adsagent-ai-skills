...
TikTok uses a separate native append contract: `append-campaign` with only `target_campaign_id`, or `append-adgroup` with only `target_adgroup_id`. A local `creative_id` is eligible only when `readiness.create_eligible=true`. When pending or historical verification rows return `next_action=creatives_reconcile`, reconcile 1..20 IDs in one bounded call and never fan out per creative. Treat terminal upload failure as requiring the returned remediation. Never translate TikTok ad groups to Meta `append-adset`, mix target IDs, auto-confirm, or replay an uncertain write.
