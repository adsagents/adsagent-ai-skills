...
- TikTok Quick Create uses native `append-campaign` with only `target_campaign_id`, or `append-adgroup` with only `target_adgroup_id`. Append-adgroup creates one Ad only. Use local `creative_id` only when `readiness.create_eligible=true`. Inspect the returned reason, retryability, formats, and next action; when asked, reconcile 1..20 tenant-owned pending or historical verification rows through one `creatives_reconcile` call and never client-side fan out. Confirm once after approval and recover uncertainty through `operations_get` on the exact original route.
