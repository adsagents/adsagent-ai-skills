...
- A paginated page 1 must be complete. Meta page 2 and later keep `consistency=cached`, `query_contract_version=1`, `require_complete_range=true`, scope, date window, timezone, grouping, filters, sorting, and page size unchanged; increment only `page`. Set `min_as_of` to task `result.meta.source_observed_at`, or for an immediate complete response use the corresponding scope overview's `query_contract.coverage.source_observed_at`. A single overview is at `data`; ordered batch overviews are at `data.items[].result`. Preserve each overview's `meta.inventory_anchor`; with multiple scopes use the earliest first-page source anchor. On a rejected continuation anchor, discard all partial rows and restart page 1 serially with the original query.
...
- `read_query_too_large` with `operator_review_required=false` is not an
  operator incident and must not be reported as one. Do not repeat the
  unchanged query. Narrow its scope/date/filter/page shape; for one known Meta
  Campaign, Ad Set, or Ad, call the matching prepare tool directly because
  prepare reads live current configuration without mutation. If explicit
  read-only proof is needed, use one typed `overview_get_live_configs` request
  without `mutation_ref`. Use `insights_export_csv` only for an explicitly
  requested large table.
...
- TikTok Quick Create uses native `append-campaign` with only `target_campaign_id`, or `append-adgroup` with only `target_adgroup_id`. Append-adgroup creates one Ad only. Use local `creative_id` only when `readiness.create_eligible=true`. Inspect the returned reason, retryability, formats, and next action; when asked, reconcile 1..20 tenant-owned pending or historical verification rows through one `creatives_reconcile` call and never client-side fan out. For an eligible local Smart+ image creative, the server verifies advertiser ownership, maps the provider image ID to `creative_info.image_info[].web_uri`, and inherits compatible image-family CTA, music, text, and landing defaults; clients must not invent provider IDs, CDN image URLs, or `CAROUSEL_ADS`. Confirm once after approval. Use `task_ref` only with `tasks_get_status`; use canonical `operation_ref` only with `operations_get` on the exact original route. `failed_proven` requires corrected input, a new prepare, and fresh approval; uncertainty is never replayed or name-matched.
- TikTok copy, structural clone, and recreate tools are capability-gated. Keep the same advertiser and exact authorization route, use one aggregate approval/task for a bounded 1..20-item grouped append, preserve item-level native receipts, and keep new delivery disabled. A failed, partial, uncertain, foreign, or unsupported source task is never replayed or matched by name.
- TikTok delivery, budget, and bid writes use only their advertised prepare/confirm names plus `operations_get`. Money is decimal advertiser-currency major units; bid is native `ad_group` scoped. Keep configured/effective/operation status, live configuration, mutation receipt, and post-mutation metrics separate.
- TikTok optimization is read-only recommendation generation over complete fresh cached evidence. Accepting a recommendation starts a new management prepare/approval flow; it is never automatic mutation. Its notifications are in-app only unless another delivery capability is explicitly advertised.
- TikTok MMP aggregate readers require `mmp_product_aggregate_reads.available=true`, are fixed server-side to `channel_pid=tiktokglobal_int`, and must not be fanned out. AppsFlyer `adset` breakdown is an adapter source field for TikTok native `ad_group`.
