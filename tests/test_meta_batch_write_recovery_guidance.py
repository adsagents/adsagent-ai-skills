...
def test_meta_batch_write_release_surfaces_are_consistently_versioned():
    assert _read("VERSION").strip() == "0.7.52"
    assert '"version": "0.7.52"' in _read(".claude-plugin/plugin.json")
    assert '"version": "0.7.52"' in _read(".claude-plugin/marketplace.json")
    assert "Current contract version: `0.7.52`" in _read("README.md")
    assert 'VERSION = "0.7.52"' in _read("scripts/validate_tri_channel_pack.py")
